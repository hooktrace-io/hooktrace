...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # only wires the SQLAlchemy instrumentation, which needs the engine
    # constructed (and the engine is itself built lazily by deps._engine()).
...
# OTEL ASGI middleware FIRST so it wraps everything else — runs at the
# outermost layer, recording one span per HTTP request with the route
# template as the span name. Same reasoning as for RateLimitMiddleware
# below: must be added at module-eval, not lifespan.
