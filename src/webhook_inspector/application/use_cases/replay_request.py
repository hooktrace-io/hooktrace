...
class ReplayRequest:
    ...
    async def execute(
        ...
        *,
        ...
        with _tracer.start_as_current_span("replay") as replay_span:
            replay_span.set_attribute("token_present", True)
            ...
            with _tracer.start_as_current_span("request.lookup"):
                ...
                if captured is None or captured.endpoint_id != endpoint.id:
                    ...
                    raise RequestNotFoundError(
                        f"request {request_id} not owned by endpoint {token}"
                    )

            # Body : inline preview or R2 fetch.
            if include_body:
                if captured.blob_key is not None:
                    with _tracer.start_as_current_span("blob.fetch") as fetch_span:
                        body = await self.blob_storage.get(captured.blob_key)
                        if body is None:
                            body = b""
                        fetch_span.set_attribute("body_bytes", len(body))
                else:
            ...
            with _tracer.start_as_current_span("ssrf.validate") as ssrf_span:
                try:
                    validated = self.target.validate(target_url)
                    ssrf_span.set_attribute("validated", True)
                except SsrfBlockedError as e:
                    ssrf_span.set_attribute("blocked", True)
                    ssrf_span.set_attribute("reason", str(e))
            ...
            # Send + persist.
            with _tracer.start_as_current_span("http.send") as send_span:
                send_span.set_attribute("method", captured.method)
                try:
                    ...
                    send_span.set_attribute("error", type(e).__name__)
                    replay = Replay.failure(
                        request_id=request_id,
                        target_url=target_url,
